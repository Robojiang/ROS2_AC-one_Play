...
class RosOperator(Node):
    def __init__(self, args, config, in_collect=False):
        ...
        if self.args.use_depth_image:
            ...
            for key, topic in depth_img_topics.items():
                try:
                    topic_path = self.config['camera_config'][topic]
                    self.get_logger().info(f"订阅深度图: {topic_path}")
                    # RealSense 深度图通常是 Image 类型，不是 CompressedImage
    ...
    def get_observation(self, ts=-1):  # get the robot observation
        ...
        for cam_name in self.args.camera_names:
            ...
            if self.args.use_depth_image:
                if cam_name in img_depth_data:
                    ...
                    # 深度图直接使用 imgmsg_to_cv2（已改为订阅 Image 类型）
                    depth_msg = deque_map[key].pop()
                    img_depth_data[key] = self.bridge.imgmsg_to_cv2(depth_msg, desired_encoding='passthrough')
        ...
        if self.args.use_depth_image:
            # 深度图 key 是 'cam_name_depth'，需要去掉 '_depth' 后缀
            obs_dict['images_depth'] = {cam.replace('_depth', ''): img for cam, img in img_depth_data.items()}
