...
def compress_and_pad_images(data_dict, camera_names, use_depth, quality=50):
    ...
    # Depth - 保持原始uint16格式，不压缩
    depth_shape = None
    if use_depth:
        # 获取深度图尺寸 (假设所有相机深度图尺寸一致)
        first_cam = camera_names[0]
        first_depth = data_dict[f'/observations/images_depth/{first_cam}'][0]
        depth_shape = first_depth.shape  # (height, width)
        
        # 转换为numpy数组 (保持uint16)
        for cam in camera_names:
            key = f'/observations/images_depth/{cam}'
            data_dict[key] = np.array(data_dict[key], dtype=np.uint16)

    return padded_size, depth_shape


def create_and_write_hdf5(args, data_dict, dataset_path, data_size, padded_size, depth_img_shape):
    with h5py.File(dataset_path + '.hdf5', 'w', rdcc_nbytes=1024 ** 2 * 2) as root:
        ...
        for cam_name in args.camera_names:
            ...
            if args.use_depth_image and depth_img_shape is not None:
                # 深度图: (data_size, height, width), 使用uint16保存原始深度值
                depth_shape = (data_size, depth_img_shape[0], depth_img_shape[1])
                depth_chunk = (1, depth_img_shape[0], depth_img_shape[1])
                depth.create_dataset(cam_name, depth_shape, 'uint16', chunks=depth_chunk)
...
def save_data(args, timesteps, actions, actions_eef, action_bases, action_velocities, ros_operator, dataset_path):
    ...
    # 压缩RGB图像数据，保留深度原始值
    padded_size, depth_shape = compress_and_pad_images(data_dict, args.camera_names, args.use_depth_image)

    # 文本的属性：
    # 1 是否仿真
    # 2 RGB图像压缩，深度图保持uint16原始格式
    t0 = time.time()
    create_and_write_hdf5(args, data_dict, dataset_path, data_size, padded_size, depth_shape)
