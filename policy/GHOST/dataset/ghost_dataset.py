...
class GHOSTDataset(BaseDataset):
    def __init__(
        self,
        zarr_path,
        horizon=1,
        ...
        if not os.path.isabs(zarr_path):
             ...
             if not os.path.exists(zarr_path):
                 ...
                 if os.path.exists(workspace_zarr_path):
                     ...
                     print(f"[GHOSTDataset] Resolved path to Workspace root: {zarr_path}")
                 else:
                     print(f"[GHOSTDataset] Warning: Zarr path not found in typical locations:")
                     print(f"  - CWD: {os.path.abspath(zarr_path)}")
                     print(f"  - Workspace: {workspace_zarr_path}")
                    
            
        print(f"[GHOSTDataset] Loading dataset from: {zarr_path}")
